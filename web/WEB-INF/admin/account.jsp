<%-- 
    Document   : account
    Created on : Mar 7, 2025, 7:05:57 PM
...
<html>
    ...
    <body>
                    ...
                    <section class="admin-main-content col-lg-10 col-md-9">
                        <div class="admin-main-content-container">
                            <div class="admin-main-content-header">
                                <div class="container">
                                    <div class="row">
                                        <h1 class="col-lg-6 col-md-6">Manage Account</h1>
                                        <div class="admin-main-content-search col-lg-6 col-md-6">
                                            <input oninput="searchAccount(this)" type="text" name="searchAccount" placeholder="Search for account name" >
                                    ...
                                    <table class="admin-main-content-body-table">
                                        <thead>
                                            <tr>
                                                ...
                                                <th>Role</th>
                                                <th>Status</th>
                                                <th class="admin-main-content-body-table-control"><button><a href="account?action=create-account"><i class="fa-solid fa-plus"></i></a></button></th>
                                            </tr>
                                        </thead>
                                        <tbody class="admin-main-content-body-tbody">
                                            <c:choose>
                                                <c:when test="${empty requestScope.listUsers}">
                                                    <tr>
                                                        <td colspan="7" style="text-align: center; font-size: 2rem;">Nothing in account found!</td>			
                                                    </tr>
                                                </c:when>
                                                <c:otherwise>
                                                    <c:forEach items="${requestScope.listUsers}" var="u">
                                                        <tr>
                                                            ...
                                                            <td>
                                                                <div >
                                                                    <img class="rounded" style="object-fit: cover" src=".${u.picture}" alt="customer img"/>
                                                                </div>
                                                            </td>
                                                            <td>${u.role}</td>
                                                            <td>${u.status}</td>
                                                            <td class="admin-main-content-body-table-control admin-account-control">
                                                                <button>
                                                                    <a href="account?action=edit-account&id=${u.id}">
                                                                        <i class="fa-solid fa-pen"></i></a>
                                                                </button>
                                                                <button onclick="handleAccount(event, 'delete-account', ${u.id})"><i class="fa-solid fa-trash"></i></button>
                                                            </td>
                                                        </tr>
                                                    </c:forEach>
                                                </c:otherwise>
                                    ...
                                    <div class="admin-main-content-body-paging">
                                        <ul>
                                            <c:forEach begin="1" end="${requestScope.numberOfPages}" var="i">
                                                <c:if test="${i == 1}"><li class="active" onclick="pagingAccount(this)">${i}</li></c:if>
                                                <c:if test="${i != 1}"><li onclick="pagingAccount(this)">${i}</li></c:if>
                                                </c:forEach>
                                        </ul>
                                    </div>
                                </div>
        ...
        <!-- box modal start -->
        <%-- Box wrong id --%>
        <div class="modall box-modal account-wrong-id">
            <div class="box-modal-container">
                <i style="color: grey" class="box-icon fa-regular fa-circle-xmark"></i>
                <h3>Fail</h3>
                <p></p>
                <button style="background-color: grey" class="bttn box-modal-btn" onclick="closeBox('create-account')">
                    <span>OK</span>
                </button>
            </div>
        </div>
        ...
        <%-- Box delete --%>
        <div class="modall box-modal account-delete-id">
            <div class="box-modal-container">
                <i style="color: orangered" class="box-icon fa-solid fa-trash"></i>
                <h3>Delete</h3>
                <p>Do you want to delete account width id <b></b>?</p>
                <div class="box-modal-control">
                    <button class="bttn box-modal-btn btn-back" onclick="closeBox('close-box', 'account-delete-id')">
                        <span>Back</span>
                    </button>
                    <button class="bttn box-modal-btn" onclick="closeBox('delete-account')">
                        <span>OK</span>
                    </button>
                </div>
            </div>
        ...
        <%-- Box delete success --%>
        <div class="modall box-modal delete-account-success">
            <h3>Success</h3>
            <div class="box-modal-container">
                <i class="box-icon fa-solid fa-circle-check"></i>
                <h3>Success</h3>
                <p>Delete successfully.</p>
                <button class="bttn box-modal-btn" onclick="closeBox('delete-account-success')">
                    <span>OK</span>
                </button>
            </div>
        </div>
        ...
        <script>
            <c:if test="${not empty requestScope.wrongID}">
                    const box_wrong_id = document.querySelector('.account-wrong-id');
