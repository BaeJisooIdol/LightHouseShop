<%-- 
    Document   : order
    Created on : Mar 14, 2025, 4:35:33 PM
...
<html>
    <head>
        ...
        <title>Admin - Manage Order Product</title>
                    ...
                    <section class="admin-main-content col-lg-10 col-md-9">
                        <div class="admin-main-content-container">
                            <div class="admin-main-content-header">
                                <div class="container">
                                    <div class="row">
                                        <h1 class="col-lg-6 col-md-6">Manage Order Product</h1>

                                        <div class="admin-main-content-search col-lg-6 col-md-6">
                                            <input oninput="searchOrder(this)" type="text" name="searchOrder" placeholder="Search for user name" >
                                        ...
                                        <tbody class="admin-main-content-body-tbody">
                                            <c:forEach items="${listOrder}" var="lo">
                                                <tr>
                                                    <td>${lo.orderID}</td>			
                                                    <td>${lo.userName}</td>
                                                    <td>${lo.productName}</td>
                                                    <td>${"x"}${lo.quantity}</td>
                                                    <td>${lo.orderDate}</td>
                                                    <td>${lo.paymentMethod}</td>
                                                    <td>${String.format("%.0f",lo.totalAmount)}</td>
                                                    <td>${lo.status}</td>
                                                    <td class="admin-main-content-body-table-control">
                                                        <c:if test="${lo.status != 'Completed'}">
                                                            <button><a href="order?action=editOrder&id=${lo.orderID}&productID=${lo.productID}"><i class="fa-solid fa-pen"></i></a></button>
                                                        </c:if>
                                                    </td>
                                                </tr>
                                            </c:forEach>
                                    ...
                                    <div class="admin-main-content-body-paging">
                                        <ul>
                                            <c:forEach begin="1" end="${requestScope.numberOfPages}" var="i">
                                                <c:if test="${i == 1}"><li class="active" onclick="pagingOrder(this)">${i}</li></c:if>
                                                <c:if test="${i != 1}"><li onclick="pagingOrder(this)">${i}</li></c:if>
                                                </c:forEach>
                                        </ul>
                                    </div>
